...
pcp_f1 = [-mp.dim, 0, -mp.dim]';
R_f2 = rpy_rot(0,0,pi);
p_f2 = [.01 .05 0];
g_f2 = tf_matrix(R_f2,p_f2);
pcp_f2 = [mp.dim, 0, -mp.dim]';
R_f3 = rpy_rot(0,0,0);
...
for i = 1:length(p_o)
    ...
    g_o(:,:,i) = tf_matrix(R_o*rpy_rot(mp.theta(i),0,0),pw_o(1:3,i)');
end
g_fingers = {g_f1 g_f2 g_f3 gmp_f3};
p_cp = [pcp_f1 pcp_f2];
p_j = mp.p_j;
p_cg = mp.p_cg;
filename = 'rolling-3D.gif';
fps=10;
hand_viz_r(mp,g_fingers,g_o,p_j,p_cg,p_cp,filename,fps,alpha)