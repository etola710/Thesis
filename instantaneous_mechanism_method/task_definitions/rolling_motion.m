...
for i=2:length(s)
    theta(i) = (s(i) - s(1))/mp.dim;
end

for i=1:length(s)
    %x = R theta
    po_cg(:,i) = [s(i);mp.dim];
    p_c(:,i) = [mp.dim*cos(theta(i)+(pi/2));mp.dim*sin(theta(i)+(pi/2))]; %starts at top of circle
    ...
    dir6 = vec2ang(po_cg(:,i),[po_cg(1,i),0]);
