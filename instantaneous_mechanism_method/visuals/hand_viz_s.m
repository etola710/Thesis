...
vectors = [f3 f18 f19 f20 f21 f22 f23 f24];
...
for i = 1:length(p_j)
    ...
    W_R = -W_23 + W_34
    W_o = [0 W_R(2) 0 0 0 0]'
    W_g = [0 0 mp.mass(3)*mp.g_acc 0 0 0]'
    %W_ext = -[0;0;W_23(3);0;0;0];
    %local wrenches to local object wrenches
    %adjoint matrix
    g34 = [eye(3) [0;mp.p_con(1);-mp.p_con(2)/2] ; zeros(1,3) 1];%local to object
    g23 = [rpy_rot(pi,0,0) [0;mp.p_con(1);mp.p_con(2)/2] ; zeros(1,3) 1];%local to object
    Adj_F34 = adjoint(g34); %transform to object wrenches
    Adj_F23 = adjoint(g23);
    Wo_F34 = Adj_F34'*W_34
    Wo_F23 = Adj_F23'*W_23
    Wo_F34(1:3) = gmp_f3(1:3,1:3)*Wo_F34(1:3)
    Wo_F23(1:3) = gmp_f3(1:3,1:3)*Wo_F23(1:3)
    Wo = Wo_F23+Wo_F34
    %object wrench due to -F23
    ...
    %{\
    p(:,:,1) = f1_j2(1:3,:); %global
    p(:,:,2) = f2_j2(1:3,:);
    %p(:,:,3) = f3_j2(1:3,:);
    %vec1 = -g_object(1:3,4,i) + p(:,:,1);
    %vec2 = -g_object(1:3,4,i) + p(:,:,2);
    Q(:,:,1) = g_object(1:3,1:3,i)*rpy_rot(0,0,0); %local contact to global
    Q(:,:,2) = g_object(1:3,1:3,i)*rpy_rot(0,0,0);
    %Q(:,:,3) = g_object(1:3,1:3,i)*rpy_rot(pi,0,0);
    %w_ext = zeros(6,1);
    %f_ext = mp.F_34(:,i); %motion in desired directions are set to 0
    %t_ext = zeros(3,1);
    %mp.w_ext(:,i) = [f_ext ; t_ext]; %external wrench
    mp.w_ext(:,i) = [-mp.F_34(:,i); zeros(3,1)];
    ...
    %axis_direction(c_f3x,c_f3y,c_f3z,g_c3,smallaxis)
    %}
    %{
    f1.XData = f3x(1);
    f1.YData = f3y(1);
    ...
    f23.YData = f1y(3);
    f23.ZData = f1z(3);
    f23.UData = -mp.w_ext(1,i)/2;
    f23.VData = -mp.w_ext(2,i)/2;
    f23.WData = -mp.w_ext(3,i)/2;
    f24.XData = f2x(3);
    f24.YData = f2y(3);
    f24.ZData = f2z(3);
    f24.UData = -mp.w_ext(1,i)/2;
    f24.VData = -mp.w_ext(2,i)/2;
    f24.WData = -mp.w_ext(3,i)/2;
    %}
    %{
    f5.XData = f3_cg1(1);
    f5.YData = f3_cg1(2);
