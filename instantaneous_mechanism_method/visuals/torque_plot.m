function mp = torque_plot(mp)
lp_sol = cell2mat(mp.x);
F_14x = lp_sol(1,:);
F_14y = lp_sol(2,:);
F_12x = lp_sol(3,:);
F_12y = lp_sol(4,:);
F_23x = -mp.mu(2)*signum(mp.svaj_curve(2,:)).*lp_sol(5,:);
F_23y = lp_sol(5,:);
F_34y = lp_sol(6,:);
F_34x = mp.mu(1)*-signum(mp.svaj_curve(2,:)).*F_34y;
T1 = lp_sol(7,:);
T2 = lp_sol(8,:);
...
for i=1:length(lp_sol)
    ...
    if (round(F_23x(i),8) <= round(mp.mu(2)*F_23y(i),8)) == 1 %friction cone constraints fx leq mu fy
        mp.FC(i) = 1;
    else
        mp.FC(i) = 0;
    end
...
figure
subplot(3,1,1)
plot(mp.tp,F_23x,mp.tp,F_23y,mp.tp,mp.mu(2)*F_23y,'--')
xlabel('Time s')
ylabel('F_{23} N')
legend('F_{23x}','F_{23y}','\mu_2 F_{23y}')
grid on
subplot(3,1,2)
plot(mp.tp,-F_23x,mp.tp,-F_23y,mp.tp,-mp.mu(2)*F_23y,'--')
xlabel('Time s')
ylabel('F_{32} N')
legend('F_{32x}','F_{32y}','\mu_2 F_{32y}')
grid on 
subplot(3,1,3)
plot(mp.tp,F_34x,mp.tp,F_34y)
xlabel('Time s')
ylabel('F_{34} N')
legend('F_{34x}', 'F_{34y}')
grid on
