...
mp.links = [.08 .05]; %m
mp.mass = [.5 .3 .05]; %kg
mp.I_fun = @(m,l) (m*l^2)/12;
mp.I = [mp.I_fun(mp.mass(1),mp.links(1)) mp.I_fun(mp.mass(2),mp.links(2))]; %moment of inertias kg m^2
%object dimensions
mp.dim = [.03 .05]; %m [height length]
mp.p_con = .03;  
mp.dt = .01;
mp.mu = [.25 .9]; %friction coefficents [object/floor , finger/object]
%gravity parameters
mp.g_acc = 9.80665;
mp.g_dir = 3*pi/2;
