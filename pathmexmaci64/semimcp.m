function [z,f,J,mu] = semimcp(z,l,u,cpfj,A,b,t,mu)
% semimcp(z,l,u,cpfj,A,b,t,mu)
%
% Solve a polyhedrally constrained variational inequality using SEMI
%
% Calling syntax: [z,f,J] = semimcp(z,l,u,cpfunjac,A,b,t,mu)
%
% Input:
%  z - starting point
%  l - lower bounds on z
...
if m > 0
  ...
  [status, ttime, f, J] = mcpsemi(row, ele, init, low, upp, 'mcp_vifunjac');
else
  [status, ttime, f, J] = mcpsemi(row, ele, init, low, upp, cpfj);
end

if (status ~= 1) 
  status,
  error('SEMI fails to solve problem');
end
